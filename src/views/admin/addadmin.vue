...
export default {
    ...
    methods: {
        ...
        async register() {
            // 非空判断
            if (this.ruleForm.user === '' || this.ruleForm.pass === ''||this.ruleForm.name === ''||this.ruleForm.phone ===''||this.ruleForm.email === '') {
              this.$message({
                message: '请将信息填写完整',
                type: 'warning',
                duration: 1000
              ...
              return; // 不执行注册逻辑
            }else if(this.ruleForm.username.slice(0,1) !== "A"){
              this.$message({
                message: '账户名不合法',
                type: 'warning',
                duration: 1000
          ...
          try {
            ...
            const response = await axios.post('http://192.168.1.115:8080/admin/register', this.ruleForm,
              {
                headers: {
            ...
            if (response.data.msg === '管理员注册成功') {
              this.$message({
                message: '注册成功',
                type: 'success',
                duration: 1000
              });
            } else if(response.data.msg === '账户已存在') {
              this.$message({
